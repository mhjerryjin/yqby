...
function processQuestion(res, req, from, to, message) {
	redisClient.get(config.redis.storage.userInputStatusPrefix + from, function(err, status){
		switch(status){
			case config.inputstatus.post_task:
				redisClient.rpush(config.redis.storage.taskDesInputPrefix + from, message);
				break;
			case config.inputstatus.wait_for_deadline:
				var deadline = +moment(message);
				if (isNaN(deadline)){
					processMessage(res, req, from, to, '时间格式无法识别，请重新输入。');
				} else {
					// TODO: 处理时间有效性
					...
					redisClient.lrange(config.redis.storage.taskDesInputPrefix + from, '0', '-1', function(err, list){
						if (list) {
							var id = uuid.v4();
							redisClient.hmset(config.redis.storage.taskPrefix + id,
								'description', JSON.stringify(list),
								'deadline', deadline,
								'creator', from
								'status', 'available');
							processMessage(res,req,from,to, '需求发布成功。');
						}
					});
				}
				break;
...
function processOtherEvent(res, req, eventKey, from, to) {
	switch (eventKey) {
		...
		case config.weixin.menu.eventkey12:
			redisClient.get(config.redis.storage.userInputStatusPrefix + from, function(err, status){
				redisClient.llen(config.redis.storage.taskDesInputPrefix + from, function(err, len){
					if(!len){
						processMessage(res, req, from, to, '您还没有输入内容。');
					} else {
						redisClient.set(config.redis.storage.userInputStatusPrefix + from, wait_for_deadline);
