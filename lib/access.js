...
function processCommentLink (res, req, from, to) {
	db.getUserAssignedQuestionDetail(from, function(status, quesid){
		if(!quesid){
			processMessage(res, req, from, to, '您还没有接受任务。');
			return;
		}
		//给接收人发消息
		db.getUser(from, function(status, user){
			if(status == 0){
				processMessage(res, req, from, to, '未能获取到您的信息。');
				return;
			}
			...
			//给发起人发消息
			db.getQuestionDetail(quesid, function(ques){
