...
function processQuestion(res, req, from, to, message) {
	switch (message.type) {
		case 1:    /*文本输入的处理*/
		{
			//往需求池子增加需求
			message.id = uuid.v4();
			db.addQuestion(from, message, function (state) {
				db.getAQuestion(from, function (status, ques) {
					if (status == 0) {
						...
						db.getQuestionDetail(ques.qid, function (result) {
							...
							custom.sendCustomMessage(ques.user, '通知：您发布的"' + quesDes + '"需求已经被小伙伴确认可以完成', function () {
...
function processOtherEvent(res, req, eventKey, from, to) {
	switch (eventKey) {
		...
		case config.weixin.menu.eventkey21:
			processQuesDetail(res,req,from,to,defaultMsg);
			break;
		// 我有疑问
...
//获取我接到的需求详情
function processQuesDetail(res, req, from, to , defaultMsg) {
	db.User(from, function(user){
		if(user && user.assigned && user.assigned.length > 0){
			//TODO: 回复所有的，现在只回复第一条
			db.getQuestionDetail(user.assigned[0], function(detail){
				processMessage(res, req, from, to, detail);
			});
		} else {
			processMessage(res, req, from, to, '您还没接收任何需求。');
		}
	});
}
