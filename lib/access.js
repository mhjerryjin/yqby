...
//处理群组回复消息
function processGroupmessage(res, req, token, userid, from, to, msgtype) {
	group.mygroupprocesscallback(token, userid, function (status, groupObj) {
		if (status == 1) {
			var msg = '请选择群组进行分享\r\n';
			if (msgtype == 3)
				msg = '您刚刚发送的语音同时将被自动识别为文字分享到明道，' + msg;
			for (var i = 0; i < groupObj.groups.length; i++) {
				msg += groupObj.groups[i].id + ' ' + groupObj.groups[i].name + '\n';
			}
			msg += '\n请回复群组编号，要分享给多个群组请将编号用英文逗号相隔进行回复！';

			processMessage(res, req, from, to, msg);
		}
		else if (status == 2)
...
function processHomepage(res, req) {
	...
	html += '<li>将您的位置信息分享到明道</li>';
	html += '<li>将微信链接网址分享到明道</li>';
...
function chatProcess(res, req) {
	var code = qs.parse(url.parse(req.url).query).code;
	var accesstokenUri = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + config.weixin.appkey + "&secret=" + config.weixin.appsecret + "&code=" + code + "&grant_type=authorization_code";
	var request = require('request');
	request.get(accesstokenUri, function (err, resp, result) {
		if (err) {
			console.log(err);

			common.Write(res, req, err, false, true);
		}
		else if (resp.statusCode == 200) {
			...
			var userinfoUri = "https://api.weixin.qq.com/sns/userinfo?access_token=" + accesstoken + "&openid=" + openid + "&lang=zh_CN";
			request.get(userinfoUri, function (err2, resp2, result2) {
				result2 = JSON.parse(result2);
				console.log(result2);

				var redirectUri = "http://wxchat.mingdao.com/?name=" + escape(result2.nickname) + "&img=" + escape(result2.headimgurl);
				//common.Write(res, req, result2, ture, true);
...
function ssoProcess(res, req) {
	var code = qs.parse(url.parse(req.url).query).code;
	var accesstokenUri = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=wxff66683c987a3b12&secret=3970bf14f90880cf6b00e0721d763d15&code=" + code + "&grant_type=authorization_code";
	var request = require('request');
	request.get(accesstokenUri, function (err, resp, result) {
		if (err) {
			console.log(err);
			...
			var accesstoken = result.access_token;
			var openid = result.openid;
			var userinfoUri = "https://api.weixin.qq.com/sns/userinfo?access_token=" + accesstoken + "&openid=" + openid;
			request.get(userinfoUri, function (err2, resp2, result2) {
				result2 = JSON.parse(result2);
				console.log(result2);

				common.Write(res, req, result2, true, true);
			});
		}
	});
};
...
exports.chat = chatProcess;
exports.sso = ssoProcess;