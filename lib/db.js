	...
	redisClient.hget(config.redis.storage.mainUserkey, userid, function (er, userInfo) {
		if (common.IsNullOrEmpty(userInfo)) {
			util.getuserdetail(userid, function (userdetail) {
				...
				var userDetail = {
					name: userdetail.nickname, //微信昵称
					avatar: userdetail.headimgurl, //微信昵称
					ques: [],	//我发布的需求ID集合
					finished: [],	//当前已经完成的需求ID集合
					assigned: ''	//当前分派到的需求编号
				};
...
/*完成任务*/
var finishedQuestion = exports.finishedQuestion = function (userid, qid, callback) {
	//获取用户详情
	getUser(userid, function (status, userDetail) {
		//增加用户与被分配需求的关联关系
		userDetail.assigned = '';
		userDetail.finished.push(qid);
...
//获取讨论
var getComment = exports.getComment = function (quesid, index, size, callback) {
...
// 插入讨论
var addComment =  exports.addComment = function (quesid, comment, callback) {
