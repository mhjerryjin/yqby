...
var assignedQuestion = exports.assignedQuestion = function (userid, qid, callback) {
	...
	redisClient.hset(config.redis.storage.assignedQueskey, qid, userid, function (err, reply) {
		//增加用户与被分配需求的关联关系
		redisClient.hget(config.redis.storage.mainUserkey, userid, function (err2, reply2) {
			if (common.IsNullOrEmpty(reply2)) {
				var userQues = {
					name: '', //微信昵称
					ques: [],	//我发布的需求ID集合
					finished: [],	//当前已经完成的需求ID集合
				...
				redisClient.hset(config.redis.storage.mainUserkey, userid, JSON.stringify(userQues), function (err3, reply3) {
					callback(1);
				});
			}
			else {
				var userQues = JSON.parse(reply2);
				userQues.assigned = qid;
